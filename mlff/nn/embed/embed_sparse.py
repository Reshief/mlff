...
class GeometryEmbedE3x(BaseSubModule):
    ...
    def __call__(self, inputs, *args, **kwargs):
        ...
        if self.input_convention == "positions":
            ...
            r_ij = jax.vmap(lambda i, j: positions[j] - positions[i])(
                idx_i, idx_j
            )  # (num_pairs, 3)
            ...
            if long_range_indices_present is True:
                ...
                r_ij_lr = jax.vmap(lambda i, j: positions[j] - positions[i])(
                    idx_i_lr, idx_j_lr
                )  # (num_pairs_lr, 3)
            ...
            if cell is not None:
                ...
                if long_range_indices_present:
                    ...
                    r_ij_lr = add_cell_offsets_sparse(
                        r_ij=r_ij_lr, cell=cell, cell_offsets=cell_offsets_lr
        ...
        # Here it is assumed that PBC (if present) have already been respected in displacement calculation.
            ...
            positions = inputs[
                "positions"
            ]  # we need positions for dipole moment calculation
            r_ij = inputs["displacements"]
            r_ij_lr = inputs.get("displacements_lr")  # shape : (num_pairs_lr, 3)
        else:
            raise ValueError(
                f"{self.input_convention} is not a valid argument for `input_convention`."
            )
...
class GeometryEmbedSparse(BaseSubModule):
    ...
    def setup(self):
        ...
        self.rbf_fn = getattr(utils.radial_basis_fn, self.radial_basis_fn)(
    ...
    def __call__(self, inputs: Dict):
                ...
                if cell is None:
                    ...
                    r_ij_lr = jax.vmap(
                        ...
                        lambda i, j: (
                            ...
                            - positions[i]
                            - jnp.round(
                                (positions[j] - positions[i]) @ jnp.linalg.inv(cell[i])
                            )
                            @ cell[i]
                        )
        ...
        rbf_ij = self.rbf_fn(
            jnp.expand_dims(d_ij, axis=-1)
        )  # shape: (num_pairs, num_radial_basis_fn)
        ...
        unit_r_ij = safe_mask(
            mask=jnp.expand_dims(d_ij, axis=-1) > 0,
            operand=r_ij,
        ...
        ylm_ij = (
            jnp.concatenate(ylm_ij, axis=-1) if len(ylm_ij) > 0 else None
        )  # shape: (num_pairs, m_tot)
        ...
        geometric_data = {
            ...
            "ylm_ij": ylm_ij,
            "ev": jnp.zeros(
                (len(inputs["atomic_numbers"]), ylm_ij.shape[-1]), dtype=r_ij.dtype
            ),
        }
...
class AtomTypeEmbedSparse(BaseSubModule):
    ...
    def __call__(self, inputs: Dict, *args, **kwargs) -> jnp.ndarray:
        ...
        return nn.Embed(num_embeddings=self.zmax + 1, features=self.num_features)(
            atomic_numbers
        )
...
class ChargeSpinEmbedSparse(nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        q = nn.Embed(num_embeddings=self.zmax + 1, features=self.num_features)(
            atomic_numbers
        )  # shape: (N,F)
        ...
        k = nn.Embed(num_embeddings=2, features=self.num_features)(psi_)[
            batch_segments
        ]  # shape: (N, F)

        v = nn.Embed(num_embeddings=2, features=self.num_features)(psi_)[
            batch_segments
        ]  # shape: (N, F)
        ...
        denominator = jax.ops.segment_sum(
            y, segment_ids=batch_segments, num_segments=len(graph_mask)
        )  # (num_graphs)

        denominator = jnp.where(
            graph_mask, denominator, jnp.asarray(1.0, dtype=q.dtype)
        )  # (num_graphs)

        a = psi[batch_segments] * y / denominator[batch_segments]  # shape: (N)
        e_psi = Residual(
            use_bias=False,
            activation_fn=getattr(jax.nn, self.activation_fn)
            if self.activation_fn != "identity"
            else lambda u: u,
