...
def make_degree_repeat_fn(degrees: Sequence[int], axis: int = -1):
    ...
    repeat_fn = partial(
        jnp.repeat, repeats=repeats, axis=axis, total_repeat_length=np.sum(repeats)
    )
    return repeat_fn


class XenophonLayerSparse(BaseSubModule):
    ...
    module_name: str = "xenophon_layer_sparse"
    ...
    def __call__(
        ...
        **kwargs,
        ...
        x_ex, ev_ex = ExchangeBlock(
            ...
            activation_fn=self.activation_fn,
            name="exchange_block",
        )(x=x, ev=ev)  # (num_nodes, num_features), (num_nodes, num_orders)
        ...
        if self.residual_mlp_2:
            y = self.activation_fn(x)  # (num_nodes, num_features)
            y = nn.Dense(features=num_features, name="res_mlp_2_layer_1")(
                y
            )  # (num_nodes, num_features)
            y = self.activation_fn(y)  # (num_nodes, num_features)
            y = nn.Dense(
        ...
        if self.layer_normalization_2:
            x = nn.LayerNorm(name="layer_normalization_2")(
                x
            )  # (num_nodes, num_features)
...
class AttentionBlock(nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        w1_ij = nn.Dense(features=num_features, name="radial_filter1_layer_2")(
            self.activation_fn(
                nn.Dense(features=num_features, name="radial_filter1_layer_1")(rbf_ij)
            )
        )  # (num_pairs, tot_num_features)

        w2_ij = nn.Dense(features=num_features, name="radial_filter2_layer_2")(
            self.activation_fn(
                nn.Dense(features=num_features, name="radial_filter2_layer_1")(rbf_ij)
            )
        )  # (num_pairs, tot_num_features)
        ...
        if self.use_spherical_filter:
            w1_ij += nn.Dense(features=num_features, name="spherical_filter1_layer_2")(
                self.activation_fn(
                    nn.Dense(
                        features=num_features // 4, name="spherical_filter1_layer_1"
                    )(contraction_fn(ev_j - ev_i))
                )
            )  # (num_pairs, tot_num_features)

            w2_ij += nn.Dense(features=num_features, name="spherical_filter2_layer_2")(
                self.activation_fn(
                    nn.Dense(
                        features=num_features // 4, name="spherical_filter2_layer_1"
                    )(contraction_fn(ev_j - ev_i))
                )
            )  # (num_pairs, tot_num_features)
        ...
        v_j = jnp.einsum("hij, Nhj -> Nhi", Wv1, x1_h)[
            idx_j
        ]  # (num_pairs, num_heads, num_features_head)
...
class ExchangeBlock(nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        y = jnp.concatenate(
            [x, contraction_fn(ev)], axis=-1
        )  # shape: (N, num_features+num_degrees)
        ...
        cx, cev = jnp.split(
            y, indices_or_sections=np.array([num_features]), axis=-1
...
class AttentionBlockDraft(nn.Module):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        w_ij = nn.Dense(features=tot_num_features, name="radial_filter_layer_2")(
            self.activation_fn(
                nn.Dense(features=tot_num_features // 2, name="radial_filter_layer_1")(
                    rbf_ij
                )
            )
        )  # (num_pairs, tot_num_features)
        ...
        if self.use_spherical_filter:
            w_ij += nn.Dense(
                features=tot_num_features, name="spherical_filter_layer_2"
            )(
                self.activation_fn(
                    nn.Dense(
                        features=tot_num_features // 2, name="spherical_filter_layer_1"
                    )(contraction_fn(ev_j - ev_i))
                )
            )  # (num_pairs, tot_num_features)
        ...
        alpha_ij = mask.safe_scale(
            (q_i * w_ij * k_j).sum(axis=-1), jnp.expand_dims(cut, axis=-1)
        )
        # (num_pairs, tot_num_heads)

        alpha1_ij, alpha2_ij = jnp.split(
            alpha_ij, indices_or_sections=np.array([self.num_heads]), axis=-1
        )
        ...
        v_j = jnp.einsum("hij, Nhj -> Nhi", Wv, x_h)[
            idx_j
        ]  # (num_pairs, num_heads, num_features_head)
