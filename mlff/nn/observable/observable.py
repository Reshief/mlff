...
from mlff.nn.activation_function.activation_function import (
    silu,
    softplus_inverse,
    softplus,
)
...
class Energy(BaseSubModule):
    prop_keys: Dict
    per_atom_scale: Sequence[float] | None = None
    per_atom_shift: Sequence[float] | None = None
    ...
    def setup(self):
        ...
        if self.per_atom_scale is not None:
            self.get_per_atom_scale = lambda y, *args, **kwargs: jnp.take(
                jnp.array(self.per_atom_scale), y
            )
            # returns array, shape: (n)
        else:
            self.get_per_atom_scale = lambda *args, **kwargs: jnp.float32(1.0)

        if self.per_atom_shift is not None:
            self.get_per_atom_shift = lambda y, *args, **kwargs: jnp.take(
                jnp.array(self.per_atom_shift), y
            )
        else:
            self.get_per_atom_shift = lambda *args, **kwargs: jnp.float32(0.0)
    ...
    def __call__(self, inputs: Dict, *args, **kwargs):
        ...
        e_loc = MLP(features=[x.shape[-1], 1], activation_fn=silu)(x).squeeze(
            axis=-1
        )  # shape: (n)
        e_loc = self.get_per_atom_scale(z) * e_loc + self.get_per_atom_shift(
            z
        )  # shape: (n)
        ...
        if self.zbl_repulsion:
            e_rep = ZBLRepulsion(
                prop_keys=self.prop_keys, output_convention=self.output_convention
            )(inputs)  # shape: (n,1) or (1)
            e_rep = e_rep - jnp.asarray(self.zbl_repulsion_shift, dtype=e_rep.dtype)
        else:
            e_rep = jnp.asarray(0.0, dtype=e_loc.dtype)  # shape: (1)
    ...
    def __dict_repr__(self) -> Dict[str, Dict[str, Any]]:
        return {
            self.module_name: {
                "per_atom_scale": self.per_atom_scale,
...
class ZBLRepulsion(nn.Module):
    ...
    def __call__(self, inputs: Dict, *args, **kwargs):
        ...
        e_rep_edge = safe_scale(w * x * y, scale=pair_mask) / jnp.asarray(
            2, dtype=d_ij.dtype
        )  # shape: (P)

        if self.output_convention == "per_atom":
            return segment_sum(e_rep_edge, segment_ids=idx_i, num_segments=len(z))[
                :, None
            ]  # shape: (n,1)
        elif self.output_convention == "per_structure":
...
def make_e_rep_fn(prop_keys, ds, cutoff_fn="cosine_cutoff_fn", r_cut=5, mic=False):
    ...
    def e_rep_fn(inputs):
        ...
        r_ij = safe_scale(
            jax.vmap(lambda i, j: R[j] - R[i])(idx_i, idx_j), scale=pair_mask[:, None]
        )
        # shape: (n_pairs,3)
        ...
        if mic:
            ...
            r_ij = add_cell_offsets(
                r_ij=r_ij, cell=cell, cell_offsets=cell_offsets
            )  # shape: (n_pairs,3)
        ...
        d_ij = safe_scale(
            jnp.linalg.norm(r_ij, axis=-1), scale=pair_mask
        )  # shape : (n_pairs)

        phi_r_cut = safe_scale(
            cut_fn(d_ij, r_cut=r_cut), scale=pair_mask
        )  # shape: (n_pairs)
...
def estimate_zbl_repulsion_contribution(prop_keys, ds, cutoff_fn, r_cut, mic=False):
    ...
    B = N_data // batch_size
    e_rep_fn = make_e_rep_fn(
        prop_keys=prop_keys, ds=ds, cutoff_fn=cutoff_fn, r_cut=r_cut, mic=mic
    )
    ...
    for b in range(B):
        train_batch = jax.tree_util.tree_map(
            lambda y: y[int(b * batch_size) : int((b + 1) * batch_size), ...], ds[0]
        )
