...
class EnergySparse(BaseSubModule):
    ...
    def __call__(self, inputs: Dict, *args, **kwargs):
        ...
        atomic_energy = jnp.where(
            theory_mask, atomic_energy, jnp.zeros_like(atomic_energy)
        ...
        if self.electrostatic_energy_bool:
            ...
            inputs.update(
                {
                    "electrostatic_energy_undamped": self.electrostatic_energy(inputs)[
        ...
        if self.dispersion_energy_bool:
            ...
            inputs.update(
                {
                    "dispersion_energy_undamped": self.dispersion_energy(inputs)[
                        "dispersion_energy"
                    ]
                }
            )
        ...
        if self.output_convention == "per_structure":
            energy = segment_sum(
                atomic_energy, segment_ids=batch_segments, num_segments=num_graphs
            )  # (num_graphs)
    ...
    def get_intermediate_quantities(self, inputs: Dict) -> Dict[str, Any]:
        ...
        if self.output_intermediate_quantities is not None:
            for imq_key in self.output_intermediate_quantities:
                ...
                if imq is None:
                    raise ValueError(
                        f"The requested intermediate quantity {imq_key} could not be generated."
                    )
...
class HirshfeldSparse(BaseSubModule):
    ...
    def __call__(self, inputs: Dict, *args, **kwargs) -> Dict[str, jnp.ndarray]:
        ...
        v_shift = nn.Embed(num_embeddings=100, features=1)(atomic_numbers).squeeze(
            axis=-1
        )  # shape: (num_nodes)
        q = nn.Embed(num_embeddings=100, features=int(num_features / 2))(
            atomic_numbers
        )  # shape: (n,F/2)
...
class PartialChargesSparse(BaseSubModule):
    ...
    def __call__(self, inputs: Dict, *args, **kwargs) -> Dict[str, jnp.ndarray]:
        ...
        # Element-dependent bias
        q_ = nn.Embed(num_embeddings=100, features=1)(atomic_numbers).squeeze(
            axis=-1
        )  # shape: (num_nodes)
        ...
        if self.regression_dim is not None:
            ...
            y = self.activation_fn(y)  # (num_nodes, regression_dim)
            x_ = nn.Dense(
                1, kernel_init=self.kernel_init, name="charge_dense_final_vec"
        ...
        if residue_charge is not None:
            ...
            batch_segments, total_charge, num_graphs = (
                residue_segments,
                residue_charge,
                residue_charge.shape[0],
            )

        # Unified charge conservation calculation
        predicted_charge = segment_sum(
            x_q, segment_ids=batch_segments, num_segments=num_graphs
        )  # (num_graphs)
        atom_counts = jnp.bincount(batch_segments, length=num_graphs)
        charge_conservation = jnp.reciprocal(atom_counts) * (
...
@jax.jit
def sigma(x):
    return safe_mask(x > 0, fn=lambda u: jnp.exp(-1.0 / u), operand=x, placeholder=0)
...
def vdw_QDO_disp_damp(
    R, gamma, C6, alpha_ij, gamma_scale, neighborlist_format: str = "sparse"
):
    ...
    V3 = (
        -C6 / (jnp.power(R, 6) + jnp.power(p, 6))
        - C8 / (jnp.power(R, 8) + jnp.power(p, 8))
        - C10 / (jnp.power(R, 10) + jnp.power(p, 10))
    )
...
def vdw_QDO_disp_damp_nosigma(
    ...
    V3 = -C6 / (jnp.power(R, 6)) - C8 / (jnp.power(R, 8)) - C10 / (jnp.power(R, 10))
...
def mixing_rules(
    ...
    alpha_i = (
        jnp.asarray(jnp.take(alphas, atomic_number_i, axis=0), dtype=dtype)
        * hirshfeld_ratio_i
    )
    C6_i = jnp.asarray(
        jnp.take(C6_coef, atomic_number_i, axis=0), dtype=dtype
    ) * jnp.square(hirshfeld_ratio_i)
    alpha_j = (
        jnp.asarray(jnp.take(alphas, atomic_number_j, axis=0), dtype=dtype)
        * hirshfeld_ratio_j
    )
    C6_j = jnp.asarray(
        jnp.take(C6_coef, atomic_number_j, axis=0), dtype=dtype
    ) * jnp.square(hirshfeld_ratio_j)

    alpha_ij = (alpha_i + alpha_j) / 2
    C6_ij = (
        2 * C6_i * C6_j * alpha_j * alpha_i / (alpha_i**2 * C6_j + alpha_j**2 * C6_i)
    )
...
def gamma_cubic_fit(alpha):
    ...
    vdW_radius = fine_structure ** (
        jnp.asarray(-4.0 / 21, input_dtype)
    ) * alpha ** jnp.asarray(1.0 / 7, input_dtype)
    ...
    sigma = (
        b3 * jnp.power(vdW_radius, 3)
        + b2 * jnp.square(vdW_radius)
        + b1 * vdW_radius
        + b0
    )
    gamma = jnp.asarray(1.0 / 2, dtype=input_dtype) / jnp.square(sigma)
    return gamma
...
def coulomb_erf(
    ...
    if cutoff is None:
        ...
        return pairwise * (
            jax.lax.erf(rij / _sigma) - jax.lax.erf(rij / (_cutoff * jnp.sqrt(2.0)))
        )
...
def coulomb_erf_shifted_force_smooth_pme(
    ...
    def force1(r, cut):
        return (
            2 * r * jnp.exp(-((r / cut) ** 2)) / (jnp.sqrt(jnp.pi) * cut)
            - jax.lax.erf(r / cut)
        ) / r**2
...
def coulomb_erf_shifted_force_smooth(
    ...
    def force(r):
        return (
            2 * r * jnp.exp(-((r / _sigma) ** 2)) / (jnp.sqrt(jnp.pi) * _sigma)
            - jax.lax.erf(r / _sigma)
        ) / r**2
...
class ZBLRepulsionSparse(BaseSubModule):
    ...
    def __call__(self, inputs: Dict, *args, **kwargs) -> Dict[str, jnp.ndarray]:
        a1 = softplus(
            self.param("a1", constant(softplus_inverse(3.20000)), (1,))
        )  # shape: (1)
        a2 = softplus(
            self.param("a2", constant(softplus_inverse(0.94230)), (1,))
        )  # shape: (1)
        a3 = softplus(
            self.param("a3", constant(softplus_inverse(0.40280)), (1,))
        )  # shape: (1)
        a4 = softplus(
            self.param("a4", constant(softplus_inverse(0.20160)), (1,))
        )  # shape: (1)
        c1 = softplus(
            self.param("c1", constant(softplus_inverse(0.18180)), (1,))
        )  # shape: (1)
        c2 = softplus(
            self.param("c2", constant(softplus_inverse(0.50990)), (1,))
        )  # shape: (1)
        c3 = softplus(
            self.param("c3", constant(softplus_inverse(0.28020)), (1,))
        )  # shape: (1)
        c4 = softplus(
            self.param("c4", constant(softplus_inverse(0.02817)), (1,))
        )  # shape: (1)
        p = softplus(
            self.param("p", constant(softplus_inverse(0.23)), (1,))
        )  # shape: (1)
        d = softplus(
            self.param("d", constant(softplus_inverse(1 / (0.8854 * self.a0))), (1,))
        )  # shape: (1)
        ...
        z_d_ij = safe_mask(
            mask=d_ij != 0, operand=d_ij, fn=lambda u: z_i * z_j / u, placeholder=0.0
        )
...
class ElectrostaticEnergySparse(BaseSubModule):
    ...
    def __call__(self, inputs: Dict, *args, **kwargs) -> Dict[str, jnp.ndarray]:
            ...
            if k_smearing is None:
                if no_sigma is not None:
                    ...
                    atomic_electrostatic_energy_ij = (
                        coulomb_erf_shifted_force_smooth_nosigma(
                            partial_charges,
                            d_ij_lr,
                            idx_i_lr,
                            idx_j_lr,
                ...
                if no_sigma is not None:
                    atomic_electrostatic_energy_ij = (
                        coulomb_erf_shifted_force_smooth_pme_nosigma(
                            partial_charges,
                            d_ij_lr,
                            idx_i_lr,
                            idx_j_lr,
        ...
        atomic_electrostatic_energy = segment_sum(
            atomic_electrostatic_energy_ij, segment_ids=idx_i_lr, num_segments=num_nodes
        )  # (num_nodes)
...
class ElectrostaticEnergyKspace(BaseSubModule):
    ...
    def setup(self):
        ...
        if self.do_ewald:
            ...
            self.solver = pme(
                get_potential(), interpolation_nodes=self.interpolation_nodes
            )
    ...
    def __call__(self, inputs: Dict, *args, **kwargs) -> Dict[str, jnp.ndarray]:
        ...
        assert positions is not None, (
            "Positions must be provided for k-space calculation."
        )
        ...
        assert k_smearing is not None, (
            "k_smearing must be provided for k-space calculation."
        )
        assert cell.shape == (3, 3), (
            f"Invalid cell shape {cell.shape}. Expected (3, 3)."
        )
        ...
        if self.do_ewald:
            potentials = self.solver.kspace(
                k_smearing, partial_charges, kvectors, positions, volume
            )
        else:
            potentials = self.solver.kspace(
                k_smearing,
...
class DispersionEnergySparse(nn.Module):
    ...
    def __call__(self, inputs: Dict, *args, **kwargs) -> Dict[str, jnp.ndarray]:
        ...
        alpha_ij, C6_ij = mixing_rules(
            atomic_numbers, idx_i_lr, idx_j_lr, hirshfeld_ratios
        )
        ...
        if self.cutoff_lr is not None:
            ...
            w = safe_mask(
                ...
                partial(
                    switching_fn,
                    x_on=self.cutoff_lr - self.cutoff_lr_damping,
                    x_off=self.cutoff_lr,
                ),
                d_ij_lr,
                0.0,
            )

            dispersion_energy_ij = safe_scale(dispersion_energy_ij, w, 0.0)

        atomic_dispersion_energy = segment_sum(
            dispersion_energy_ij, segment_ids=idx_i_lr, num_segments=num_nodes
        )  # (num_nodes)
