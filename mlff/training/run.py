...
def run_training(state: TrainState,
    ...
    for i in range(1, int(steps_per_epoch * epochs) + 1):
        ...
        train_batch = jax.tree.map(lambda y: y[perms[step_in_epoch], ...], train_ds)
        if loss_fn_input is not None:
            loss_input_batch = jax.tree.map(lambda y: y[perms[step_in_epoch], ...], loss_fn_input)
        ...
        if (np.isnan(train_batch_metrics_np['loss']) or
            ...
            def reset_records():
                x = jax.tree.map(lambda x: jnp.zeros(x.shape), state.params['record'])
                y = jax.tree.map(lambda x: jnp.zeros(x.shape), state.valid_params['record'])
                return x, y
