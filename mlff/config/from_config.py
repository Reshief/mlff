...
from mlff.nn.stacknet.stacknet_sparse import StackNetSparse
...
def make_so3krates_sparse_from_config(
    ...
    return nn.SO3kratesSparse(
        ...
        avg_num_neighbors=config.data.avg_num_neighbors
        ...
        electrostatic_energy_kspace_do_ewald_bool=getattr(
            config, "electrostatic_energy_kspace_do_ewald_bool", None
        ),
        electrostatic_energy_kspace_interp_nodes=getattr(
            config, "electrostatic_energy_kspace_interp_nodes", 4
        ),
...
def make_itp_net_from_config(config: config_dict.ConfigDict) -> StackNetSparse:
...
def make_optimizer_from_config(
    ...
    if config is None:
        ...
        return training_utils.make_optimizer(
            ...
            optimizer_args=opt_config.optimizer_args
            if opt_config.optimizer_args is not None
            else dict(),
            ...
            if opt_config.learning_rate_schedule_args is not None
            else dict(),
            num_of_nans_to_ignore=opt_config.num_of_nans_to_ignore,
            gradient_clipping=opt_config.gradient_clipping,
            gradient_clipping_args=opt_config.gradient_clipping_args
            if opt_config.gradient_clipping_args is not None
            else dict(),
        )
...
def run_training(config: config_dict.ConfigDict, model: str = "so3krates"):
    ...
    training_data, validation_data, data_stats = prepare_training_and_validation_data(
        config=config, loader=loader, tf_record_present=tf_record_present
    )
    ...
    if config.training.batch_max_num_nodes is None:
        ...
        batch_max_num_nodes = (
            data_stats["max_num_of_nodes"] * (config.training.batch_max_num_graphs - 1)
            + 1
        )
        batch_max_num_edges = (
            data_stats["max_num_of_edges"] * (config.training.batch_max_num_graphs - 1)
            + 1
        )
    ...
    if config.training.batch_max_num_pairs is None:
        if config.data.neighbors_lr_bool is True:
            ...
            batch_max_num_pairs = (
                data_stats["max_num_of_nodes"]
                * (data_stats["max_num_of_nodes"] - 1)
                * (config.training.batch_max_num_graphs - 1)
                + 1
            )
...
def run_evaluation(
    ...
    targets = (
        ...
        else list(config.training.loss_weights.keys())
    )
    ...
    if num_test is not None:
        if num_test > num_data:
            raise RuntimeError(
                f"num_test = {num_test} > num_data = {num_data} in data set {data_filepath}."
            )
    ...
    if not tf_record_present:
        testing_data = data.transformations.subtract_atomic_energy_shifts(
            ...
            atomic_energy_shifts={
                int(k): v for (k, v) in config.data.energy_shifts.items()
            },
        )
    else:
    ...
    if "params" in params and "observables_0" in params["params"]:
        ...
        if "energy_offset" in params["params"]["observables_0"]:
            ...
            if len(old_energy_offset.shape) == 1:
                new_energy_offset = jnp.tile(
                    old_energy_offset[:, None], (1, num_theory_levels)
                )
        ...
        if "atomic_scales" in params["params"]["observables_0"]:
            ...
            if len(old_atomic_scales.shape) == 1:
                new_atomic_scales = jnp.tile(
                    old_atomic_scales[:, None], (1, num_theory_levels)
                )
        ...
        if "energy_dense_final" in params["params"]["observables_0"]:
            ...
            old_kernel = params["params"]["observables_0"]["energy_dense_final"][
                "kernel"
            ]
            ...
            if old_kernel.shape[1] == 1:
                ...
                params["params"]["observables_0"]["energy_dense_final"]["kernel"] = (
                    new_kernel
                )
...
def run_fine_tuning(
    ...
    if strategy == "full":
        ...
        trainable_subset_keys = [
            f"layers_{config_start_from_workdir.model.num_layers - 1}"
        ]
    ...
    with open(workdir / "fine_tuning.json", mode="w") as fp:
        json.dump(
            {"start_from_workdir": start_from_workdir.as_posix(), "strategy": strategy},
            fp=fp,
        )
    ...
    if trainable_subset_keys is not None:
        opt = training_utils.freeze_parameters(
            optimizer=opt, trainable_subset_keys=trainable_subset_keys
        )
    ...
    logging.mlff(f"Fine tuning model from {start_from_workdir} on {data_filepath}!")
...
def data_loader_from_config(config):
    ...
    if hasattr(config.data, "datasets"):
        ...
        dataset_paths = [
            Path(d["path"]).expanduser().resolve() for d in config.data.datasets
        ]
        dataset_weights = [d.get("weight", 1.0) for d in config.data.datasets]
        tf_record_present = all(
            len([1 for x in os.scandir(p) if Path(x).suffix[:9] == ".tfrecord"]) > 0
    ...
    if tf_record_present:
        ...
        if data_filepath.is_file():
            ...
            npz_record_present = (
                len([1 for x in os.scandir(data_filepath) if Path(x).suffix == ".npz"])
                > 0
            )
            ...
            raise ValueError(
                f"Data path {data_filepath} does not exist or is not accessible"
            )
...
def prepare_training_and_validation_data(
    config, loader, tf_record_present, model_config: Optional = None
):
    # Lock the config.
    config = config.lock()
    ...
    if not tf_record_present:
        ...
        # data loader.
        training_and_validation_indices = np.sort(
            all_indices[: (num_train + num_valid)]
        )
        ...
        training_data = [
            training_and_validation_data[i_train] for i_train in internal_train_indices
        ]
        validation_data = [
            training_and_validation_data[i_val] for i_val in internal_validation_indices
        ]
        ...
        validation_indices = training_and_validation_indices[
        ...
        if (
            config.training.batch_max_num_nodes is None
            or config.training.batch_max_num_edges is None
        ):
            raise ValueError(
        ...
        if (
            config.data.neighbors_lr_bool is True
            and config.training.batch_max_num_pairs is None
        ):
            raise ValueError(
    ...
    if config.data.shift_mode == "mean":
        ...
        energy_mean = (
            data.transformations.calculate_energy_mean(training_data) * energy_unit
        )
        num_nodes = data.transformations.calculate_average_number_of_nodes(
            training_data
        )
        ...
        if config.data.energy_shifts is None:
            raise ValueError(
                "For config.data.shift_mode == custom config.data.energy_shifts must be given."
            )
    else:
        config.data.energy_shifts = {str(a): 0.0 for a in range(119)}
    # And lock again.
    config = config.lock()

    if not tf_record_present:
        training_data = list(
            data.transformations.subtract_atomic_energy_shifts(
                data.transformations.unit_conversion(
                    training_data,
                    energy_unit=energy_unit,
                    length_unit=length_unit,
