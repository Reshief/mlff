...
def test_so3krates_init():
    ...
    n_layers = 4
    net = nn.So3krates(n_layer=n_layers, prop_keys=prop_keys)
    ...
    def check_layer(u):
        ...
        assert u["parity"]
        assert (
            u["fb_rad_filter_features"]
            == SO3KRATES_DEFAULT_ARGS["fb_rad_filter_features"]
        )
        assert (
            u["gb_rad_filter_features"]
            == SO3KRATES_DEFAULT_ARGS["gb_rad_filter_features"]
        )
        assert (
            u["fb_sph_filter_features"]
            == SO3KRATES_DEFAULT_ARGS["fb_sph_filter_features"]
        )
        assert (
            u["gb_sph_filter_features"]
            == SO3KRATES_DEFAULT_ARGS["gb_sph_filter_features"]
        )
...
def test_so3krates_init_with_kwargs():
    ...
    net = nn.So3krates(F=F, so3krates_layer_kwargs=test_args, prop_keys=prop_keys)
...
def test_so3krates_param_init():
    ...
    inputs = {
        prop_keys[pn.atomic_position]: jax.random.normal(
            jax.random.PRNGKey(0), shape=(7, 3)
        ),
...
def test_so3krates_training():
    ...
    data_set.random_split(
        n_train=50, n_valid=10, n_test=None, r_cut=r_cut, training=True, seed=0
    )
    ...
    inputs = jax.tree.map(lambda x: jnp.array(x[0, ...]), train_ds[0])
    ...
    train_state, h_train_state = create_train_state(
        ...
        plateau_lr_decay={"patience": 50, "decay_factor": 0.5},
        scheduled_lr_decay={
            "exponential": {"transition_steps": 10_000, "decay_factor": 0.9}
        },
        lr_warmup={"init_value": 0, "peak_value": 1, "warmup_steps": 10},
    )
